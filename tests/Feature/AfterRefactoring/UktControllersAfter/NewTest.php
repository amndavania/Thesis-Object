...
class NewTest extends TestCase
{
    ...
    public function test_store_ukt_with_automatic_examcard_and_bimbinganstudy_generation()
    {
        ...
        $response->assertRedirect(route('ukt.index')); //Tujuan pengujian 5 → Sistem melakukan redirect ke halaman index setelah penyimpanan berhasil
        $this->assertDatabaseHas('ukts', [ //Tujuan pengujian 1 → UKT baru berhasil disimpan dengan status Lunas
            'students_id' => $this->student->id,
            'year' => 2024,
            'semester' => 'GASAL',
            'type' => 'UKT',
        ...
        $this->assertDatabaseHas('exam_card', [ //Tujuan pengujian 2 → Sistem otomatis membuat ExamCard UTS dan UAS
            'students_id' => $this->student->id,
            'type' => 'UTS',
            'semester' => 'GASAL',
            'year' => 2024,
        ...
        $this->assertDatabaseHas('bimbingan_study', [ //Tujuan pengujian 3 → Sistem otomatis membuat Bimbingan Study
            'students_id' => $this->student->id,
            'semester' => 'GASAL',
            'year' => 2024,
        ]);
        ...
        $this->assertDatabaseHas('transactions', [ //Tujuan pengujian 4 → Sistem mencatat transaksi debit & kredit
            'reference_number' => '123456',
            'type' => 'debit',
        ]);
    ...
    {
        $controller = new \App\Http\Controllers\TransactionServiceController();
    ...
    public function test_set_keterangan_lunas_generates_examcards_and_bimbingan()
    {
        ...
        $controller->setKeterangan($studentData, 2024, 'GENAP', 'UKT', 'Lunas', $ukt); //setTotalStatus 1

        $this->assertDatabaseHas('exam_card', ['students_id' => $this->student->id, 'type' => 'UTS']);
        $this->assertDatabaseHas('exam_card', ['students_id' => $this->student->id, 'type' => 'UAS']);
        $this->assertDatabaseHas('bimbingan_study', ['students_id' => $this->student->id]);
    }
    ...
    public function test_set_keterangan_lunas_uts_sets_exam_or_keterangan()
    {
        ...
        $controller->setKeterangan($studentData, 2024, 'GENAP', 'UKT', 'Lunas UTS', $ukt); //setTotalStatus 2
        $this->assertDatabaseHas('exam_card', ['type' => 'UTS', 'students_id' => $this->student->id]);
    ...
    public function test_set_keterangan_lunas_krs_sets_keterangan_or_lbs()
    {
        ...
        $controller->setKeterangan($studentData, 2024, 'GENAP', 'UKT', 'Lunas KRS', $ukt); //setTotalStatus 3
        $this->assertDatabaseHas('bimbingan_study', ['students_id' => $this->student->id]);

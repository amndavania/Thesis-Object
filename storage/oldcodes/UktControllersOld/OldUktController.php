...
class UktController extends Controller
{
     ...
     */
    public function store(UktCreateRequest $request) //this
    {
    ...
    public function setKeterangan($studentData, $year, $semester, $payment_type, $status) //this
    {
        $payment = Ukt::latest()->first(); //ukt
        ...
        if ($payment_type == "UKT") {
            ...
            $bimbinganStudy = BimbinganStudy::where('students_id', $studentData[0]->id) //bimbinganstudy
            ...
            if ($status == "Lunas") {
                if (!$bimbinganStudy) {
                    ...
                    // $payment->keterangan = 'UAS';
                    $exam_uts = ExamCard::where('students_id', $studentData[0]->id)->where('semester', $semester)->where('type', "UTS")->exists(); //examcard

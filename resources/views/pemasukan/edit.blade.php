     ...
     <div class="card-body">
        @include('message.form-message')
          ...
          <div class="form-group">
               ...
               <input type="number" class="form-control" id="amount" name="amount" placeholder="Jumlah..." value="{{ old('amount', $pemasukan->amount) }}">
          </div>
          <div class="form-group">
          ...
          <select class="form-control" id="transaction_accounts_id" name="transaction_accounts_id">
               @foreach ($transaction_account as $item)
                    <option value="{{ $item->id }}" {{ $pemasukan->transaction_accounts_id == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
               @endforeach
          </select>
          </div>
